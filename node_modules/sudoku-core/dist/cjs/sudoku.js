...
exports.createSudokuInstance = void 0;
...
const GROUP_OF_HOUSES = (0, utils_1.generateHouseIndexList)(constants_1.BOARD_SIZE);
// Function to create a new Sudoku instance
function createSudokuInstance(options = {}) {
    const { onError, onUpdate, onFinish, initBoard, difficulty = constants_1.DIFFICULTY_MEDIUM, } = options;
    let board = [];
    ...
    const resetCandidates = () => {
        board = new Array(constants_1.BOARD_SIZE * constants_1.BOARD_SIZE).fill(null).map((_, index) => (Object.assign(Object.assign({}, board[index]), { candidates: board[index].value == null
                ? constants_1.CANDIDATES.slice()
    ...
    const initializeBoard = () => {
        ...
        if (!alreadyEnhanced) {
            ...
            board = Array.from({ length: constants_1.BOARD_SIZE * constants_1.BOARD_SIZE }, (_, index) => {
                var _a;
                const value = (_a = initBoard === null || initBoard === void 0 ? void 0 : initBoard[index]) !== null && _a !== void 0 ? _a : null;
                const candidates = value == null ? [...constants_1.CANDIDATES] : [...constants_1.NULL_CANDIDATE_LIST];
    ...
    const housesWithCell = (cellIndex) => {
        const boxSideSize = Math.sqrt(constants_1.BOARD_SIZE);
        const groupOfHouses = [];
        //horizontal row
        const horizontalRow = Math.floor(cellIndex / constants_1.BOARD_SIZE);
        groupOfHouses.push(horizontalRow);
        //vertical row
        const verticalRow = Math.floor(cellIndex % constants_1.BOARD_SIZE);
    ...
    const getRemainingNumbers = (house) => {
        ...
        return constants_1.CANDIDATES.filter((candidate) => !usedNumbers.includes(candidate));
    };
    /* getUsedNumbers
     * --------------
    ...
    function openSinglesStrategy() {
        ...
        for (let i = 0; i < groupOfHouses.length; i++) {
            for (let j = 0; j < constants_1.BOARD_SIZE; j++) {
                const singleEmptyCell = findSingleEmptyCellInHouse(groupOfHouses[i][j]);
                if (singleEmptyCell) {
                    return fillSingleEmptyCell(singleEmptyCell);
                }
                if ((0, utils_1.isBoardFinished)(board)) {
                    onFinish === null || onFinish === void 0 ? void 0 : onFinish((0, utils_1.calculateBoardDifficulty)(usedStrategies, strategies));
                    return true;
                }
            }
        }
    ...
    function findSingleEmptyCellInHouse(house) {
        const emptyCells = [];
        for (let k = 0; k < constants_1.BOARD_SIZE; k++) {
    ...
    function fillSingleEmptyCell(emptyCell) {
        ...
        (0, utils_1.addValueToCellIndex)(board, emptyCell.cellIndex, value[0]); //does not update UI
    ...
    * -----------------------------------------------------------------*/
    function updateCandidatesBasedOnCellsValue() {
        ...
        for (let houseType = 0; houseType < groupOfHousesLength; houseType++) {
            for (let houseIndex = 0; houseIndex < constants_1.BOARD_SIZE; houseIndex++) {
                const house = GROUP_OF_HOUSES[houseType][houseIndex];
                const candidatesToRemove = getUsedNumbers(house);
                for (let cellIndex = 0; cellIndex < constants_1.BOARD_SIZE; cellIndex++) {
    ...
    const convertInitialBoardToSerializedBoard = (_board) => {
        return new Array(constants_1.BOARD_SIZE * constants_1.BOARD_SIZE).fill(null).map((_, i) => {
            ...
            for (let houseIndex = 0; houseIndex < constants_1.BOARD_SIZE; houseIndex++) {
                ...
                for (let digitIndex = 0; digitIndex < digits.length; digitIndex++) {
                    ...
                    for (let cellIndex = 0; cellIndex < constants_1.BOARD_SIZE; cellIndex++) {
                        ...
                        if ((0, utils_1.contains)(boardCell.candidates, digit)) {
                    ...
                    if (possibleCells.length === 1) {
                        ...
                        (0, utils_1.addValueToCellIndex)(board, cellIndex, digit);
            ...
            for (let houseIndex = 0; houseIndex < constants_1.BOARD_SIZE; houseIndex++) {
                ...
                for (let digitIndex = 0; digitIndex < digits.length; digitIndex++) {
                    ...
                    for (let cellIndex = 0; cellIndex < house.length; cellIndex++) {
                        const cell = house[cellIndex];
                        if ((0, utils_1.contains)(board[cell].candidates, digit)) {
    ...
    function nakedCandidatesStrategy(number) {
        ...
        function checkCombinedCandidates(house, startIndex) {
            for (let i = Math.max(startIndex, minIndexes[startIndex]); i < constants_1.BOARD_SIZE - number + startIndex; i++) {
        ...
        function checkLockedCandidates(house, startIndex) {
            ...
            for (let i = Math.max(startIndex, minIndexes[startIndex]); i <= constants_1.BOARD_SIZE - number + startIndex; i++) {
                ...
                if (combineInfo.length > 0) {
                    ...
                    for (let a = 0; a < combineInfo.length; a++) {
                        ...
                        for (let b = 0; b < cells.length; b++) {
                            if (!(0, utils_1.contains)(temp, cells[b]))
                                temp.push(cells[b]);
                        }
                    }
                    if (temp.length > number) {
                ...
                if (combineInfo.length === number) {
                    ...
                    for (let c = 0; c < constants_1.BOARD_SIZE; c++) {
                        if (!(0, utils_1.contains)(combinedCandidates, c + 1))
    ...
    const setBoardCellWithRandomCandidate = (cellIndex) => {
        ...
        const value = (0, utils_1.getRandomCandidateOfCell)(candidates);
        (0, utils_1.addValueToCellIndex)(board, cellIndex, value);
        return true;
    };
    const invalidPreviousCandidateAndStartOver = (cellIndex) => {
        ...
        (0, utils_1.addValueToCellIndex)(board, previousIndex, null);
        resetCandidates();
    ...
    const generateBoardAnswerRecursively = (cellIndex) => {
        if (cellIndex + 1 > constants_1.BOARD_SIZE * constants_1.BOARD_SIZE) {
    ...
    function isValidAndEasyEnough(analysis, difficulty) {
        return (analysis.hasSolution &&
            analysis.difficulty &&
            (0, utils_1.isEasyEnough)(difficulty, analysis.difficulty));
    }
    // Function to prepare the game board
    const prepareGameBoard = () => {
        const cells = Array.from({ length: constants_1.BOARD_SIZE * constants_1.BOARD_SIZE }, (_, i) => i);
        let removalCount = (0, utils_1.getRemovalCountBasedOnDifficulty)(difficulty);
        while (removalCount > 0 && cells.length > 0) {
            ...
            (0, utils_1.addValueToCellIndex)(board, cellIndex, null);
            // Reset candidates, only in model.
            resetCandidates();
            const boardAnalysis = analyzeBoard();
            if (isValidAndEasyEnough(boardAnalysis, difficulty) &&
                (0, sudoku_solver_1.isUniqueSolution)(getBoard())) {
                removalCount--;
            }
            else {
                // Reset - don't dig this cell
                (0, utils_1.addValueToCellIndex)(board, cellIndex, cellValue);
            }
        }
    };
    ...
    function analyzeBoard() {
        ...
        const data = {
            hasSolution: (0, utils_1.isBoardFinished)(board),
            usedStrategies: filterAndMapStrategies(strategies, usedStrategies),
        };
        if (data.hasSolution) {
            const boardDiff = (0, utils_1.calculateBoardDifficulty)(usedStrategies, strategies);
    ...
    function generateBoard() {
        ...
        function isBoardTooEasy() {
            ...
            if (data.hasSolution && data.difficulty) {
                return !(0, utils_1.isHardEnough)(difficulty, data.difficulty);
            }
            return true;
        }
    ...
    const solveStep = ({ analyzeMode = false, iterationCount = 0, } = {}) => {
        ...
        if (!(0, utils_1.isBoardFinished)(board) && boardNotChanged) {
...
exports.createSudokuInstance = createSudokuInstance;
